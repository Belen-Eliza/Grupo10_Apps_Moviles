...
export const UserContextProvider = ({
  ...
  const cambiarNombre = async (nombre_nuevo: string) => {
    try {
      const rsp = await fetch(
        `${process.env.EXPO_PUBLIC_DATABASE_URL}/users/edit_profile/${id}`,
        {
          method: "PATCH",
  ...
  const actualizar_info = async (id: number) => {
    //después de editar el perfil o agregar gastos e ingresos
    try {
      const rsp = await fetch(
        `${process.env.EXPO_PUBLIC_DATABASE_URL}/users/${id}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
